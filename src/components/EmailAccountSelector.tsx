...
export default function EmailAccountSelector({ onAccountChange, onShowConnectionModal }: Props) {
  ...
  const loadAccounts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Notifier le parent du compte actif
  ...
  const getConnectionStatus = () => {
    ...
    if (!active) return { text: 'Déconnecté', color: 'text-gray-500' };
    
    return {
      text: `Connecté à ${active.provider.name}`,
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center space-x-2 px-3 py-1.5 bg-white/10 rounded-lg hover:bg-white/20 text-white"
      >
        {activeAcc ? (
          <div className={`w-5 h-5 rounded-full ${activeAcc.provider.color} flex items-center justify-center text-white text-xs`}>
      ...
      {showDropdown && (
        <div className="absolute top-full left-0 mt-1 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
          <div className="p-3 border-b">
            <h3 className="font-medium text-gray-900">Comptes email</h3>
          </div>
              ...
              accounts.map((account) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full ${account.provider.color} flex items-center justify-center text-white`}>
          ...
          <div className="p-3 border-t space-y-2">
            <button
              ...
              className="w-full flex items-center space-x-2 px-3 py-2 text-blue-600 hover:bg-blue-50 rounded"
            >
            ...
            <button
              ...
              className="w-full flex items-center space-x-2 px-3 py-2 text-gray-600 hover:bg-gray-50 rounded"
            >
